...
Open a terminal instance inside the root directory containing your docker-compose.yml file.

Execute the following command to completely purge legacy volumes, rebuild application layers, and start the system in detached mode:

Bash
sudo docker compose down -v && sudo docker compose up --build -d
Infrastructure Lifecycle Steps
The engine starts the primary kafka-server instance and monitors its built-in health check.

Once healthy, the kafka-init worker mounts, creates warehouse-processing-lane (3 partitions) and warehouse-dead-letter-queue (1 partition), and exits cleanly.

The three C# services initialize concurrently and hook directly into the prepared topics.
